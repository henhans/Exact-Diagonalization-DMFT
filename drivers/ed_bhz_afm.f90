  ...
  program ed_bhz_afm
    ...
    logical                :: waverage,spinsym,getak,wbroyden
    ...
    if(wbroyden)allocate(Amix(Nindep,Nspin,Norb,Lmats),Amix_old(Nindep,Nspin,Norb,Lmats))
    ...
    do while(.not.converged.AND.iloop<nloop)
       ...
       do ip=1,Nindep
          if(ED_MPI_ID==0)write(LOGfile,*)"Solving site:",ip
          ed_file_suffix="_is"//reg(txtfy(ip))
          Hloc = bhzHloc_site(ip)
       ...
       call get_delta
       if(wbroyden.AND.ED_MPI_ID==0)then
          Amix_old=Amix
...
#ifdef _MPI
          call MPI_BCAST(Delta,size(Delta),MPI_Double_Complex,0,MPI_COMM_WORLD,mpiERR)
#endif          
       endif
  ...
  contains
    ...
    subroutine get_delta
      ...
      if(getak)then
         if(ED_MPI_ID==0)print*,"Get A(k,w):"
         do ip=1,Nindep
            do iorb=1,Norb
               unit=free_unit()
         ...
         do i=1,Lreal
            ...
            do ik=1,Lk
               ...
               enddo
            enddo
            if(ED_MPI_ID==0)call progress(i,Lreal)
         enddo
    ...
    subroutine build_hk(file)
      ...
      if(getak)then
         ...
         do ix=1,Nk
            ...
            if(ED_MPI_ID==0)write(unit,"(I,16F25.12)")ik,(eig(i),i=1,afmNso)
         enddo

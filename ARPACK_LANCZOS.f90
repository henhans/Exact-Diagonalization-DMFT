...
contains
  ...
  subroutine lanczos_arpack_c(ns,neigen,nblock,nitermax,eval,evec,hprod,iverbose)
    ...
    interface
       subroutine hprod(n,vin,vout)
         integer                 :: n
         complex(8),dimension(n) :: vin,vout
       end subroutine hprod
    end interface
  ...
  !    MORE DETAILS ABOUT THE DRIVER ARE GIVEN BELOW:
  !+-------------------------------------------------------------------+
  subroutine lanczos_parpack(ns,neigen,nblock,nitermax,eval,evec,hprod,iverbose)
    implicit none
    !Arguments
    integer             :: ns,neigen,nblock,nitermax
    ...
    !Interface to Matrix-Vector routine:
    interface
       subroutine hprod(Q,R,nloc,n,vin,vout)
         integer                 :: Q,R,nloc,n
    ...
    if(info<0)then
       ...
       !    nconv =  iparam(5)
       !    do j = 1, nconv
       !       call hprod(mpiQ,mpiR, ldv,n, v(1,j), ax )
       !       call daxpy( n, -d(j,1), v(1,j), 1, ax, 1 )
       !       d(j,2) = dnrm2(n,ax,1)
       !       d(j,2) = d(j,2) / abs ( d(j,1) )
       !    end do
